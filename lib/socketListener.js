...
var multiwii = require('multiwii'),
  ...
  setServoOffset = require('./setServoOffset');

var events = ['rc', 'ident', 'status', 'pid', 'rawMotorEnabled', 'servoOffset'];
...
var SocketListener = exports.SocketListener = function (socket, wii) {
  ...
  this.readPid = this.readPid.bind(this);
  this.epromWrite = this.epromWrite.bind(this);
  this.setRawMotorEnabled = this.setRawMotorEnabled.bind(this);
  this.setServoOffset = this.setServoOffset.bind(this);
...
SocketListener.prototype.connect = function(device, cb) {
  ...
  wii.connect(device).then(function () {
    ...
    wii.addInMessage('setServoOffset', setServoOffset);
    ...
    self.socket.on('pid', self.readPid);
    self.socket.on('setServoOffset', self.setServoOffset);
    self.socket.on('epromWrite', self.epromWrite);
...
//  console.log('setRawRc', roll, pitch, yaw, throttle);
  var self = this;
...
SocketListener.prototype.setPid = function(pid) {
  var self = this;
  this.wii.setPid(pid.roll, pid.pitch, pid.yaw, pid.alt, pid.pos, pid.posr, pid.navr, pid.level, pid.mag, pid.vel)
    .then(function () {
...
SocketListener.prototype.readPid = function() {
  this.wii.read('pid');
};

SocketListener.prototype.epromWrite = function() {
  var self = this;
  this.wii.epromWrite()
    .then(function () {
      console.log('wrote to eprom');
    })
    .catch(function (err) {
      console.error('eprom write error', err);
    });
};
...
SocketListener.prototype.setAccCalibration = function() {
  var self = this;
  console.log('setAccCalibration')
  this.wii.setAccCalibration();
};

SocketListener.prototype.setServoOffset = function (left, right) {
  this.wii.setServoOffset(left, right);
};
